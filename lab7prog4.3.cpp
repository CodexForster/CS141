...
class Rectangle {
	...
	private:
		int side1(Point A, Point C);
		int side2(Point A, Point C);
	public:
		void area(Point A, Point C);
...
int Rectangle::side1(Point A, Point C){
	Point B;
	B.Set_X(A.Get_X());
	B.Set_Y(C.Get_Y());
	return B.Get_Y() - A.Get_Y();
} 
int Rectangle::side2(Point A, Point C){
	Point B;
	B.Set_X(A.Get_X());
	B.Set_Y(C.Get_Y());
	return C.Get_X() - B.Get_X();
} 
void Rectangle::area(Point A, Point C){
	int s1,s2;
	s1 = side1(A,C);
	s2 = side2(A,C); 
	cout << "\n Area of the rectangle is " << s1*s2;
} 
